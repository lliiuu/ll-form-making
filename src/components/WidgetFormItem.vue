<template>
  <el-form-item class="widget-view " v-if="element && element.key"
    :class="{ active: selectWidget.key == element.key, 'is_req': element.options.required }" :label="element.name"
    @click.native.stop="handleSelectWidget(index)">
    <span slot="label">
      <span class="span-box">
        <span>{{ element.name }}</span>
        <i v-if="element.tooltip" style="color: #FF9800;font-size: 18px;cursor:pointer;" class="el-icon-question"></i>
      </span>
    </span>
    ...
    <template v-if="element.type == 'textarea'">
      <el-input type="textarea" :rows="element.options.rows" v-model="element.options.defaultValue"
        :style="{ width: element.options.width + '%' }" :disabled="element.options.disabled"
        :placeholder="element.options.placeholder"></el-input>
    </template>
    <template v-if="element.type == 'table'">
      <el-table :data="element.options.options" style="width: 100%">
        <el-table-column v-for="(item, index) in element.options.labelOptions" :key="item.value + index"
    ...
    <template v-if="element.type == 'radio'">
      <el-radio-group v-model="element.options.defaultValue" :style="{ width: element.options.width + '%' }">
        <el-radio :style="{ display: element.options.inline ? 'inline-block' : 'block' }" :label="item.value"
    ...
    <template v-if="element.type == 'checkbox'">
      <el-checkbox-group v-model="element.options.defaultValue" :style="{ width: element.options.width + '%' }">
        <el-checkbox :style="{ display: element.options.inline ? 'inline-block' : 'block' }" :label="item.value"
          v-for="(item, index) in element.options.options" :key="item.value + index">
          {{ element.options.showLabel ? item.label : item.value }}
        </el-checkbox>
      </el-checkbox-group>
    ...
    <template v-if="element.type == 'time'">
      <el-time-picker v-model="element.options.defaultValue" :is-range="element.options.isRange"
        ...
        :arrowControl="element.options.arrowControl" :value-format="element.options.format"
    ...
    <template v-if="element.type == 'selectCreate'">
      <el-select v-model="element.options.defaultValue" :disabled="element.options.disabled"
        :multiple="element.options.multiple" :clearable="element.options.clearable"
        :placeholder="element.options.placeholder" :style="{ width: element.options.width + '%' }">
        <el-option v-for="item in element.options.options" :key="item.value" :value="item.value"
    ...
    <template v-if="element.type == 'switch'">
      <el-switch active-value="true" inactive-value="false" v-model="element.options.defaultValue"
        :disabled="element.options.disabled">
      </el-switch>
    </template>

    <template v-if="element.type == 'slider'">
      <el-slider v-model="element.options.defaultValue" :show-input="element.options.showInput"
        :range="element.options.range" disabled :style="{ width: element.options.width + '%' }"></el-slider>
    </template>
    ...
    <template v-if="element.type == 'fileupload'">
      <el-upload v-model="element.options.defaultValue" :disabled="element.options.disabled"
        :style="{ 'width': element.options.width + '%' }" drag :action="element.options.action" multiple>
        <i class="el-icon-upload"></i>
        <div class="el-upload__text">{{ element.options.uploadText }}</div>
      </el-upload>
    </template>

    <template v-if="element.type == 'outFileupload'">
      <el-upload v-model="element.options.defaultValue" :disabled="element.options.disabled"
        :style="{ 'width': element.options.width + '%' }" drag :action="element.options.action" multiple>
        <i class="el-icon-upload"></i>
        <div class="el-upload__text">{{ element.options.uploadText }}</div>
    ...
    <template v-if="element.type == 'dir'">
      <el-input v-model.trim="element.options.defaultValue" :disabled="element.options.disabled"
        :placeholder="element.options.placeholder" :style="{ width: element.options.width + '%' }">
    ...
    <template v-if="element.type == 'cascader'">
      <el-cascader v-model="element.options.defaultValue" :disabled="element.options.disabled"
        ...
        :style="{ width: element.options.width + '%' }" :options="element.options.remoteOptions">

      </el-cascader>
    </template>

    <template v-if="element.type == 'editor'">
      <fm-editor v-model="element.options.defaultValue" :width="element.options.width + '%'"
    ...
    <el-button title="删除" @click.stop="handleWidgetDelete(index)" class="widget-action-delete"
      v-if="selectWidget.key == element.key && element.delAble" circle plain type="danger">
    ...
    <el-button title="复制" @click.stop="handleWidgetClone(index)" class="widget-action-clone"
      v-if="selectWidget.key == element.key && element.delAble" circle plain type="primary">
...
export default {
  ...
  // mounted () {
  //   let oldoOptions = JSON.parse(JSON.stringify(this.element.options.options));
  //   if(this.element.type === 'select' && this.element.options.remote==='add'){
  //     let batchArr = this.element.options.batchString.split(',').map(itemChild => ({value:itemChild,label:itemChild}));
  //     this.element.options.options = batchArr;
  //   }else if(this.element.type === 'select' && this.element.options.remote===false){
  //     this.element.options.options = oldoOptions;
  //   }
  // },
  methods: {
