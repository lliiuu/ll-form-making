<template>
    <el-container>
        <el-aside class="c-d-aside">
            <div class="components-list">
                <div class="widget-cate">布局元素</div>
                <draggable element="ul" :list="layoutComponents" :options="{
                ...
                <div class="widget-cate">输入元素</div>
                <draggable element="ul" :list="basicComponents" :options="{
                    group: { name: 'people', pull: 'clone', put: false },
                    sort: false,
                    ghostClass: 'ghost',
                }" @end="handleMoveEnd" @start="handleMoveStart" :move="handleMove">
                    <li class="form-edit-widget-label" v-for="(item, index) in basicComponents" :key="index">
                        <a>
                            <i v-if="item.type != 'table' && item.type != 'dir'" class="icon iconfont"
                                :class="item.icon"></i>
                            <i v-else class="icon" :class="item.icon"></i>
                            <span v-if="item.type != 'selectCreate'">{{
                                    item.name
                            }}</span>
                            <span v-else>账号设置</span>
                        </a>
                    </li>
                </draggable>

                <!-- <div class="widget-cate">输入高级元素</div>
        ...
        <el-container class="center-container" direction="vertical">
            <el-header class="btn-bar" style="height: 45px">
                ...
                <el-button v-if="upload" type="text" size="medium" icon="el-icon-upload2"
                    @click="handleUpload">导入JSON</el-button>
        ...
        <el-aside class="widget-config-container" style="width: 260px">
            <el-container>
                ...
                <el-main class="config-content">
                    <widget-config v-show="configTab == 'widget'" :data="widgetFormSelect"
                        :list-data="widgetForm"></widget-config>
                    <form-config v-show="configTab == 'form'" :data="widgetForm.config"></form-config>
                </el-main>
            </el-container>
        </el-aside>

        <cus-dialog :visible="previewVisible" @on-close="previewVisible = false" ref="widgetPreview"
            @on-submit="handleTest" width="1000px" form>
            <generate-form insite="true" v-if="previewVisible" :sm2Public="sm2Public" :data="widgetForm"
                :remote="remoteFuncs" :value="widgetModels" :uploadType="uploadType" ref="generateForm">
            ...
            <div v-if="isHaveWideget" style="text-align: center">
                暂无数据...
            </div>
        </cus-dialog>
            ...
            this.$refs.generateForm
                ...
                .then((data) => {
                    if (JSON.stringify(data) == '{}') {
                        this.$alert('暂无数据...', '表单数据').catch((e) => { })
                    } else {
                        this.$alert(data, '表单数据').catch((e) => { })
                    }
